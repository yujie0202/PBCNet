...
class DMPNN_Encoder(nn.Module):
    ...
    def forward(self, G, gm):
        ...
        message = self.act_func(self.W_i(feats))  # num_bonds x hidden_size
...
def func(device = 'cuda:0', label_scalar = None, loss_fn = "smoothl1", continue_learning = True,retrain=False):

    if label_scalar == None:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results3", f"DMPNNres_gai_{loss_fn}_scalarFalse_continue{continue_learning}")
    else:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results3", f"DMPNNres_gai_{loss_fn}_scalarTrue_continue{continue_learning}")
    ...
    train_dataloader = GraphDataLoader(train_dataset, collate_fn=collate_fn, batch_size=256, drop_last=False, shuffle= (not continue_learning),num_workers=4,pin_memory = True)
    valid_dataloader = GraphDataLoader(valid_dataset, collate_fn=collate_fn, batch_size=256, drop_last=False, shuffle=True,num_workers=4,pin_memory = True)
    ...
    if retrain == True:
        model = torch.load("/home/yujie/leadopt/results/DMPNNres_pair_evidential_scalarTrue_continueTrue/model18.pth")
    else:
        if loss_fn == 'mve':
    ...
    opt = torch.optim.Adam(model.parameters(), lr = 0.0001, eps = 1e-4)
    ...
    for epoch in range(9999):
        ...
        for k in train_dataloader:
            ...
            opt.zero_grad()
            loss.backward()
            # print(param_count_gradient(model))
        ...
        with torch.no_grad():
            ...
            csv_save_dir = os.path.join(SAVEDIR, f"scalar_corr_{EPOCH}.csv")
