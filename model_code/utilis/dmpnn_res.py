...
def func(device = 'cuda:0', label_scalar = None, loss_fn = "smoothl1", continue_learning = True,retrain=False):
    ...
    if label_scalar == None:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results3", f"DMPNNres_pair_{loss_fn}_scalarFalse_continue{continue_learning}")
    else:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results3", f"DMPNNres_pair_{loss_fn}_scalarTrue_continue{continue_learning}")
