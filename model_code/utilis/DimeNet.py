...
def param_count_gradient(model: nn.Module) -> int:
    ...
    :return: The number of trainable parameters.
    """
    return sum(torch.mean(torch.abs(param)) for param in model.parameters() if param.requires_grad)
...
def get_parser():
    ...
    parser.add_argument(
        '--loss_func', type=str, default="smoothl1",
...
def collate_fn(samples):
    ...
    ligand1_atomnum_list = [Extend(i, list(range(ligand1_num - len(i)))) for i in
                            ligand1_atomnum_list]  # 二维 [[1,3,2],[0,0,1]]
...
class LeadOptDataset:
    def __init__(self, df_path, label_scalar=None):
        ...
        if self.label_scalar is not None:
...
class DMPNN(nn.Module):
    def __init__(self, hidden_dim, num_bilinear, act, num_spherical, num_radial,
                 num_before_skip, num_after_skip, radius, T, p_dropout,
                 ffn_num_layers, output_dim=1):
        super(DMPNN, self).__init__()

        self.encoder = DimeNet(hidden_dim=hidden_dim, num_bilinear=num_bilinear, radius=radius,
                               act=act, num_spherical=num_spherical, num_radial=num_radial,
        ...
        ffn = [nn.Linear(hidden_dim * 3, hidden_dim), nn.LeakyReLU(0.2)]
        for _ in range(ffn_num_layers - 1):
            ffn.append(nn.Linear(hidden_dim, hidden_dim))
            ffn.append(nn.LeakyReLU(0.2))
...
def func(device='cuda:0', label_scalar=None, loss_fn="smoothl1", continue_learning=True):
    if label_scalar == None:
        SAVEDIR = os.path.join(os.environ["HOME"], "leadopt", "results3",
                               f"Dime_swish_gai_{loss_fn}_scalarFalse_continue{continue_learning}")
    else:
        SAVEDIR = os.path.join(os.environ["HOME"], "leadopt", "results3",
                               f"Dime_swish_gai_{loss_fn}_scalarTrue_continue{continue_learning}")
    ...
    train_dataloader = GraphDataLoader(train_dataset, collate_fn=collate_fn, batch_size=90, drop_last=False,
                                       shuffle=(not continue_learning), num_workers=4, pin_memory=True)
    valid_dataloader = GraphDataLoader(valid_dataset, collate_fn=collate_fn, batch_size=90, drop_last=False,
