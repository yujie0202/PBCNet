...
class ResidualLayer(torch.nn.Module):
    ...
    def forward(self, x):
        return x + self.dropout_layer(self.act(self.lin2(self.act(self.lin1(x)))))
...
class DMPNN_Encoder(nn.Module):
    ...
    def forward(self, G, gm):
        ...
        for _ in range(self.num_MPNN_layer-1):
            ...
            message = message_clone.index_add_(0, idx_ji, message[idx_kj]) - message_clone
...
def func(device = 'cuda:0', label_scalar = None, loss_fn = "smoothl1", continue_learning = True):
    ...
    if label_scalar == None:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results", f"DMPNN_pair_{loss_fn}_scalarFalse_continue{continue_learning}")
    else:
        SAVEDIR = os.path.join(os.environ["HOME"],"leadopt", "results", f"DMPNN_pair_{loss_fn}_scalarTrue_continue{continue_learning}")
    ...
    train_dataloader = GraphDataLoader(train_dataset, collate_fn=collate_fn, batch_size=150, drop_last=False, shuffle= (not continue_learning),num_workers=4,pin_memory = True)
    valid_dataloader = GraphDataLoader(valid_dataset, collate_fn=collate_fn, batch_size=150, drop_last=False, shuffle=True,num_workers=4,pin_memory = True)
    ...
    if loss_fn == 'mve':
        hidden_dim, radius, T, p_dropout, ffn_num_layers, output_dim 
